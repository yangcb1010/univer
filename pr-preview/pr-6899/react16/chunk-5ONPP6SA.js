      ...
      this._sheetInterceptorService.interceptCommand({
        getMutations: (commandInfo) => {
          ...
          if (commandInfo.id === RemoveSheetCommand.id) {
            ...
            const workbook = params.unitId ? this._univerInstanceService.getUnit(params.unitId) : this._univerInstanceService.getCurrentUnitForType(2 /* UNIVER_SHEET */);
            if (!workbook) {
              return { redos: [], undos: [] };
            }
...
function CellLinkPopupPure(props) {
  var _a, _b;
  const popupService = useDependency(SheetsHyperLinkPopupService);
  const commandService = useDependency(ICommandService);
...
var getShouldDisableCurrentCellLink = (accessor) => {
  const unit = accessor.get(IUniverInstanceService).getCurrentUnitForType(2 /* UNIVER_SHEET */);
  if (!unit) {
    return true;
  }

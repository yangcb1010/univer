...
var SheetNumfmtUIController = class extends Disposable {
  ...
  _forceUpdate(unitId) {
    ...
    const renderUnit = this._renderManagerService.getRenderById(
      unitId != null ? unitId : this._univerInstanceService.getCurrentUnitForType(2 /* UNIVER_SHEET */).getUnitId()
    );
        ...
        getMutations(command) {
          ...
          switch (command.id) {
            case SetRangeValuesCommand.id: {
              const workbook = self._univerInstanceService.getCurrentUnitForType(2 /* UNIVER_SHEET */);

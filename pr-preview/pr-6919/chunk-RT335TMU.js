...
var SheetsFilterService = class extends Disposable {
  ...
  _updateActiveFilterModel() {
    let workbook;
    try {
      workbook = this._univerInstanceService.getCurrentUnitForType(2 /* UNIVER_SHEET */);
      if (!workbook) {
...
var SmartToggleSheetsFilterCommand = {
  ...
  handler: async (accessor) => {
    ...
    const sheetsFilterService = accessor.get(SheetsFilterService);
    const commandService = accessor.get(ICommandService);
    const currentWorkbook = univerInstanceService.getCurrentUnitForType(2 /* UNIVER_SHEET */);
